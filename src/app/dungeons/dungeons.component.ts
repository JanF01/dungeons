...
export class DungeonsComponent implements OnInit {
  ...
  constructor(private audio: AudioService, private dungeons: DungeonsService, private images: ImagesService){
    this.player = {
      ...
      new Weapon("normal","A Fricking Sword","assets/weapon/1_2.png","#00A11","none",Math.round(2*2*0.8),(Math.round(2*12)/10)+2,(Math.round(2*12)/10)+2+Math.round(2/4.5),100,Math.random()*30-15,"normal"),
      new Weapon("normal","A Fricking Sword","assets/weapon/1_2.png","#00A11","none",Math.round(2*2*0.8),(Math.round(2*12)/10)+2,(Math.round(2*12)/10)+2+Math.round(2/4.5),100,Math.random()*30-15,"normal")],
      armorsInShop: [new Armor("normal","Shield Yourself","assets/armor/1_1.png","#00B12","none",Math.round(2*2),Math.round(2*2/2)+2+30,5+Math.round(Math.random()*5),100,Math.random()*30-15,"normal"),
